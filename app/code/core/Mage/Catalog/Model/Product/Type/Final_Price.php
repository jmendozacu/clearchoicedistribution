...
class Mage_Catalog_Model_Product_Type_Price
{
    ...
    public function getBasePrice($product, $qty = null)
    {
		if($qty == null){
			...
			$productId = $product->getIdBySku($product->sku);
			$_product		=	Mage::getModel('catalog/product')->load($productId);
			if($_product->getSpecialPriceCaseQty()>0){
				$castQty			=	 $_product->getSpecialPriceCaseQty();
				$qty1				=	 floor($qty/$castQty);
				$withtier			=	$qty1*$castQty;
				$withouttier		=	$qty-$withtier;
				$price = (float)$product->getPrice();
				$withoutierprice	=	$price*$withouttier;
					
				$withtiergotprice	=	 min($this->_applyGroupPrice($product, $price), $this->_applyTierPrice($product, $withtier, $price),
					$this->_applySpecialPrice($product, $price)
				);
				
				$withtierprice		= 	$withtiergotprice*$withtier;
				$total				=	($withtierprice + $withoutierprice)/$qty;
				return $total;
			}
			else{
				$price = (float)$product->getPrice();
    ...
    public function getTierPrice($qty = null, $product)
    {
        ...
        $query			=	"select count(cat_id) as existance,cat_id from multipletier where product_sku='$sku'";
        $result 		=	$read->fetchAll($query);
        $mainCat		=	$result[0]['cat_id'];
        $tierQty		=	0;
		$productId 		= 	$product->getIdBySku($sku);
		$_product		=	Mage::getModel('catalog/product')->load($productId);
		$mainProductCase=	$_product->getSpecialPriceCaseQty();
		if($result[0]['existance']>0 && $qty!=0){
			$session = Mage::getSingleton('checkout/session');
			$output = "";
			if($mainProductCase>0){
				foreach ($session->getQuote()->getAllItems() as $item) {
					...
					$_product		=	Mage::getModel('catalog/product')->load($productId);				
					if($_product->getSpecialPriceCaseQty()>0){
						if($result[0]['existance']>0 && $result[0]['cat_id']==$mainCat)
						{			
						$caseQty		=	$_product->getSpecialPriceCaseQty();
						$cases			=	floor($item->getQty()/$caseQty);					
						$applicableQty	=	$cases*$mainProductCase; //consider cases according to main product
						$tierQty		=	$tierQty+$applicableQty;
						}
					}
				}
			}
