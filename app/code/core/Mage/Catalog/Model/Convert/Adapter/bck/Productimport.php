        ...
        if (($tier_prices_field) && !empty($tier_prices_field)) {
            ...
            if(trim($tier_prices_field) == 'REMOVE'){
                ...
                foreach($existing_tps as $key => $etp){
                    $etp_lookup[intval($etp['customer_group_id'])][intval($etp['price_qty'])] = $key;
                }
...
foreach($incoming_tierps as $tier_str){
                    ...
                    if(isset($etp_lookup[1][intval($tmp[0])])){
                        unset($existing_tps[$etp_lookup[1][intval($tmp[0])]]);
                    }
...
foreach($incoming_tierps as $tier_str){
                    ...
                    //drop any existing tier values by qty
                    if(isset($etp_lookup[2][intval($tmp[0])])){
                        unset($existing_tps[$etp_lookup[2][intval($tmp[0])]]);
                    }
                    
                } 
				$tps_toAdd_array = array();
               
				foreach($tps_toAdd as $key =>$val)
				{
					foreach($val as $key1 =>$val1)
					{
						$tps_toAdd_array [] = $val1;
					}
				}
				foreach($existing_tps as $key =>$val)
				{
					foreach($val as $key1 =>$val1)
					{
						$tps_toAdd_array [] = $val1;
               ...
               // $tps_toAdd =  array_merge($existing_tps, $tps_toAdd);
				//save it
                $product->setTierPrice($tps_toAdd_array);
                //$product->setData('tier_price', $tps_toAdd);                              
            }
    ...
    public function saveRow( array $importData )    
    {
        ...
        if(!empty($importData['mptp_cat_id'])):
			...
			if(count($catExistance)==0):
				// if category does'nt exist then create a category. Please check categroy under multiple tier in admin.
				$connection1					= 	Mage::getSingleton('core/resource')->getConnection('core_write');
				$query1								=	"insert into multiple_tier_cat (`cat_name`)
														values('$mptpCatId')";
				$res1 								= 	$connection1->query($query1);
			endif;		
		...
		if( !empty($importData['short_name'])){
        ...
        if(isset($importData['MAP']) || !empty($importData['MAP']) || isset($importData['map']) || !empty($importData['map'])){
			$product->setMap();
		}
		if(isset($importData['power_consumption']) || !empty($importData['power_consumption'])){
			$product->setPowerConsumption();
		}
		if(isset($importData['product_weight']) || !empty($importData['product_weight'])){
			$product->setProductWeight();
		}
		if(isset($importData['compatible_products']) || !empty($importData['compatible_products'])){
			$product->setCompatibleProducts();
		}
...
//testcode ends here
       ...
       /* if(
        ...
        /* This code is for Deleting Previous images
         * of product if it already exists.
         * 1stly we check if product exists and then delete its previous  images
         * */
         /* 
		$productIdCheck = $product -> getIdBySku( $importData['sku'] );
        if ($productIdCheck) {
				if ($product->getId()){
        ...
        if ( !empty( $importData['gallery'] ) ) {
				...
				$counter		=	0;
					foreach( $galleryData as $gallery_img ) {
						try {
							//echo Mage :: getBaseDir( 'media' ) . DS . 'import' . $gallery_img;
							if($counter==0){
								if((empty( $importData['thumbnail'] ) && empty($importData['small_image'] )) || ($importData['thumbnail']==$importData["image"] && $importData['small_image']==$importData["image"])){		
									$visibility1 = array (
														'thumbnail',
														'small_image',
														'image'
													);
									$product -> addImageToMediaGallery( Mage :: getBaseDir( 'media' ) . DS . 'import' . $gallery_img, $visibility1, false, false );
									$shouldInclude = 'no';
								}
								else{
            ...
            if ( !empty( $importData['small_image'] ) && $shouldInclude=='yes') {
            ...
            if ( !empty( $importData['thumbnail'] ) && $shouldInclude=='yes') {
            ...
            foreach( $galleryData as $gallery_img ) {
                try {
						...
						if($gallery_img==$importData["image"]){
							...
							if($gallery_img==$importData["small_image"] || $gallery_img==$importData["thumbnail"]){							
								if($gallery_img==$importData["small_image"] && $shouldInclude=='yes'){
									$visibility1 = array (
									'small_image',
									NULL
									);
								...
								if($gallery_img==$importData["thumbnail"] && $shouldInclude=='yes'){
									$visibility1 = array (
									'thumbnail',
									NULL
									);
