    ...
    */
    public function saveRow( array $importData )    
    {
        ...
        if(!empty($importData['mptp_cat_id'])):  
			...
			$query 						= "SELECT * FROM multiple_tier_cat where `cat_id`='$mptpCatId'";
			...
			$connection					= 	Mage::getSingleton('core/resource')->getConnection('core_read');
			$query 						= "SELECT * FROM mptp where `product_sku`='$currentProductSku'";
			$existanceStatus			=	$connection->fetchAll($query);			
		     if(
            isset($importData['ProductGroup_Quantity_1'])
            && !empty($importData['ProductGroup_Quantity_1'])
            && isset($importData['ProductGroup_Price_1'])
            && !empty($importData['ProductGroup_Price_1'])
            ){

				$quantityRange			=	'ProductGroup_Quantity_';
				$priceVal				=	'ProductGroup_Price_';
				$count					=	1;
				$mptpUpdateStatus		= 	$importData['mptp_update_status'];
				$mptpUpdationController	=	'deactivated';
				while(!empty($importData[$quantityRange.$count]) && !empty($importData[$priceVal.$count])){
						$flag								=	0;
						//$_Pdetails 							=	Mage::getModel('catalog/product')->loadByAttribute('sku',$productSku);
						$interval							=	array();
						$interval							=	explode('-',$importData[$quantityRange.$count]);
						$from								=	$interval[0];
						$to									=	$interval[1];
						$priceOfCurrentProduct				=	$importData[$priceVal.$count];
						if($mptpUpdationController =='activated'):
							$writeConnection 				= 	Mage::getSingleton('core/resource')->getConnection('core_write');
							$query							=	"insert into mptp (`cat_id`,`from`,`to`,`product_sku`,`price_per_product`)
																values($mptpCatId,$from,$to,'$currentProductSku',$priceOfCurrentProduct)";
							$res 							= 	$writeConnection->query($query);						
						else:
							if($mptpUpdateStatus==1):
								$writeConnection 					= 	Mage::getSingleton('core/resource')->getConnection('core_write');
								$query								=	"delete from mptp where `product_sku`= '$currentProductSku'";
								$res 							= 	$writeConnection->query($query);
								$writeConnection 					= 	Mage::getSingleton('core/resource')->getConnection('core_write');
								$query								=	"insert into mptp (`cat_id`,`from`,`to`,`product_sku`,`price_per_product`)
																		values($mptpCatId,$from,$to,'$currentProductSku',$priceOfCurrentProduct)";
								$res 							= 	$writeConnection->query($query);												
								$mptpUpdationController			=	'activated';
							else:
							
									if(count($existanceStatus)==0):
										$writeConnection 					= 	Mage::getSingleton('core/resource')->getConnection('core_write');
										$query								=	"insert into mptp (`cat_id`,`from`,`to`,`product_sku`,`price_per_product`)
																				values($mptpCatId,$from,$to,'$currentProductSku',$priceOfCurrentProduct)";
										$res 							= 	$writeConnection->query($query);
						...
						//echo $priceOfCurrentProduct;die;
						//$connection							= 	Mage::getSingleton('core/resource')->getConnection('core_read');
						//$query12 							= "SELECT * FROM mptp where `product_sku`='$currentProductSku' and `from`=$from and `to`=$to and `price_per_product`=$priceOfCurrentProduct";
						//$existanceStatus					=	$connection->fetchAll($query12);
						//print_r($existanceStatus);			
						//if(count($existanceStatus)==0){
							//echo count($existanceStatus);
							//$writeConnection 				= 	Mage::getSingleton('core/resource')->getConnection('core_write');
							//$query12							=	"insert into mptp (`from`,`to`,`product_id`,`price_per_product`)
																//values($from,$to,'$productCalculatedId',$priceOfCurrentProduct)";
							//$res 							= 	$writeConnection->query($query12);
						//}
							//else{
								/*
								//echo count($existanceStatus);
								...
								$query						=	"delete from mptp where product_id='$productCalculatedId'";
								$res 						= 	$writeConnection->query($query);
								
								$writeConnection->commit();

								$writeConnectioninsert			= 	Mage::getSingleton('core/resource')->getConnection('core_write');
								$writeConnectioninsert->beginTransaction();
								$query12						=	"insert into mptp (`from`,`to`,`product_id`,`price_per_product`)
																	values($from,$to,'$productCalculatedId',$priceOfCurrentProduct)";
								$res 						= 	$writeConnectioninsert->query($query12);	
								$writeConnectioninsert->commit();		*/				
						//}
						$flag++;					
					$count++;
				}
		...
		if ( isset(	$importData['limited_stock_qty']))
		 ...
		 if ( isset($importData['Normal']) || isset($importData['Wholesale']))
		 {
				...
				if(isset($importData['bundle_attributes'])){
					...
					if(isset($importData['bundle_attribute_products'])){
						...
						if(!$productId = $product->getIdBySku($importData['sku'].'_base')){
							...
							$productBase->setTaxClassId(2); # My default tax class
						...
						if($product->getIdBySku($importData['sku'])) die;
						//This loop will process product set of on attribute example: ['LT121|No|0|Yes|0|0',LT121|No|0|Yes|0|0]
						...
						$selectionRawData[$attributeCounter+1][$productCounter] 	= array(
															  'product_id' => $product -> getIdBySku($importData['name'].'_base'),
															  'selection_qty' => 1,
															  'selection_can_change_qty' => 0,
															  'position' => 0,
															  'is_default' => 1,
