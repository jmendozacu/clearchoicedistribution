    ...
    public function saveRow( array $importData )    
    {
        ...
        if(!empty($importData['mptp_cat_id'])):  
			...
			$existanceStatus			=	$connection->fetchAll($query);			
		     if(
            isset($importData['Quantity_range_1'])
            && !empty($importData['Quantity_range_1'])
            && isset($importData['Price_1'])
            && !empty($importData['Price_1'])
            ){
				$quantityRange			=	'Quantity_range_';
				$priceVal				=	'Price_';
				$count					=	1;
				...
				while(!empty($importData[$quantityRange.$count]) && !empty($importData[$priceVal.$count])){
						...
						if($mptpUpdationController =='activated'):
							...
							$query							=	"insert into multipletier (`cat_id`,`from`,`to`,`product_sku`,`price_per_product`)
       ...
       /* if(
        ...
        if(
            isset($importData['Quantity_1'])
            && !empty($importData['Quantity_1'])
            && isset($importData['our_price_1'])
            && !empty($importData['our_price_1'])
            ){
				$columnName			=	'our_price_';
				$count				=	1;
				$cloneTier			=	array();
				while(!empty($importData[$columnName.$count])){
					$calculatedString		=	$importData['Quantity_'.$count].'='.$importData[$columnName.$count];
