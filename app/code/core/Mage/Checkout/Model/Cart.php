...
class Mage_Checkout_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    ...
    /* This Method has been added by PatrickS, To add multiple products
     * 
     * 
     * 
     * 
     */
    public function addProductsByIdsAddOn($productIds)
    {
        $allAvailable = true;
        $allAdded     = true;
        ...
        if (!empty($productIds)) {
            foreach ($productIds as $productId) {
                $productIdN 		= 	(int) $productId['id'];
                $productQty			=	(int) $productId['qty'];
                if (!$productIdN) {
                    continue;
                }
                $product = $this->_getProduct($productIdN);
                if ($product->getId() && $product->isVisibleInCatalog()) {
                    try {
                        $this->getQuote()->addProduct($product,$productQty);
                    } catch (Exception $e){
                        $allAdded = false;
                    }
                } else {
