...
if ( !$_SERVER["HTTP_USER_AGENT"] ) { // to run via local browser use if ($_SERVER["SERVER_ADDR"] == $_SERVER["REMOTE_ADDR"]) {
    $profile_id = 13; // SYSTEM - IMPORT/EXPORT - ADVANCED PROFILES
    $store_id = 1; // used when create grouped products only
    $website_id = 1; // used when create grouped products only
                ...
                try {
                    ...
                    try {
                        ...
                        if ( $importData['status'] == 'Enabled' ) {
                            if ( $importData['type'] == 'simple' ) { // tierprices por simple products
                                $product_id = $write -> fetchOne( "select * from $product_table where sku='" . $importData['sku'] . "'" );
                                $customer_groups = $write -> query( "select * from $customer_group_table" );
                                foreach ( $customer_groups as $customer_group ) {
                                    if ( $customer_group['customer_group_code'] != 'General' && $customer_group['customer_group_code'] != 'NOT LOGGED IN' ) {
                                        try {
                                            if ( $write -> fetchOne( "select * from $product_entity_tier_price_table where entity_id=" . ( int )$product_id . " and all_groups=0 and customer_group_id=" . ( int )$customer_group['customer_group_id'] . " and qty=1 and website_id=0" ) ) {
                                                $write -> query( "update $product_entity_tier_price_table set value=" . $importData[$customer_group['customer_group_code']] . " where entity_id=" . ( int )$product_id . " and all_groups=0 and customer_group_id=" . ( int )$customer_group['customer_group_id'] . " and qty=1 and website_id=0" );
                                                } else {
                                                $write -> query( "insert into $product_entity_tier_price_table (entity_id, all_groups, customer_group_id, qty, value, website_id) values (" . ( int )$product_id . ", 0, " . ( int )$customer_group['customer_group_id'] . ", 1, " . $importData[$customer_group['customer_group_code']] . ", 0)" );
                                                } 
                                            } 
                                        catch ( Exception $e ) {
                            ...
                            if ( $importData['type'] == 'grouped' ) { // for grouped products
                                ...
                                foreach ( explode( ',', $importData['associated'] ) as $product_sku ) {
                                    ...
                                    try {
                                        ...
                                        $attribute_id = $write -> fetchOne( "select * from $attribute_table where attribute_code='" . $importData['config_attributes'] . "'" );
                                        if ( !$write -> fetchOne( "select * from $index_eav_table where store_id=" . $store_id . " and entity_id=" . ( int )$parent_id . " and attribute_id=" . ( int )$attribute_id ) ) {
                                            $write -> query( "insert into $index_eav_table (store_id, entity_id, attribute_id, value) values (" . $store_id . ", " . ( int )$parent_id . ", " . $attribute_id . ", " . $valueCount++ . ")" );
                                            } 
                                        /**
                                        * WE ARE NOT USING CATALOG PRICE RULES
                                        * $customer_groups = $write->query("select * from $customer_group_table");
                                        * foreach ($customer_groups as $customer_group) {
                                        * if (!$write->fetchOne("select * from $index_minimal_price_table where entity_id=".(int)$parent_id." and customer_group_id=".(int)$customer_group['customer_group_id']." and tax_class_id=".(int)$customer_group['tax_class_id']." and website_id=".$website_id)) {
                                        * $write->query("insert into $index_minimal_price_table (entity_id, customer_group_id, tax_class_id, website_id) values (".(int)$parent_id.", ".(int)$customer_group['customer_group_id'].", ".(int)$customer_group['tax_class_id'].", ".$website_id.")");
                                        * }
                                        * if (!$write->fetchOne("select * from $index_price_table where entity_id=".(int)$parent_id." and attribute_id=".$price_attribute." and customer_group_id=".(int)$customer_group['customer_group_id']." and tax_class_id=".(int)$customer_group['tax_class_id']." and website_id=".$website_id)) {
                                        * $write->query("insert into $index_price_table (entity_id, attribute_id, customer_group_id, value, tax_class_id, website_id) values (".(int)$parent_id.", ".$price_attribute.", ".(int)$customer_group['customer_group_id'].", ".(int)$importData[$customer_group['customer_group_code']].", ".(int)$customer_group['tax_class_id'].", ".$website_id.")");
                                        * }
                                        * }
                                        */
                                        } 
